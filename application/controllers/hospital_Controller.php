...

{
    ...
    public function hospost()
    {
        ...
        //     $config['upload_path'] = 'userUploads/images/';
    ...
    public function addSymptoms()
    {
        ...
        $addSymptoms = $this->hospitalView_Model->addsymptoms($data, $p_id);
        if ($addSymptoms) {
        ...
        if (!$this->input->post()) {
            ...
            if ($this->form_validation->run()) {
                ...
                if (!$this->upload->do_upload('picture')) {
                    ...
                    $config2['upload_path'] = './userUploads/';
                ...
                if (!$this->upload->do_upload('signature')){
                    ...
                    $fdata = $this->upload->data();
                     $sign_image_path = base_url('userUploads/' . $fdata['file_name']);
                }
                    ...
                    $data = array(
                        ...
                        'state_medical_council' => $this->input->post('state_medical_council'),
                        ...
                        'specialization' => $this->input->post('Other_specialization') ? $this->input->post('Other_specialization') : $this->input->post('specialization'),

                        'picture' => $image_path,

                        'signature' => $sign_image_path,

                        'department' => $this->input->post('department'),

                        'degree' => $this->input->post('Others') ? $this->input->post('Others') : $this->input->post('degree'),

                        'speciality' => $this->input->post('Other_speciality') ? $this->input->post('Other_speciality') : $this->input->post('speciality'),
                    ...
                    // $result = $this->doctors_Model->get_state_council($state_medical_council, $registration_num);
    ...
    //                     </script>";
                        ...
                        //window.location.href='http://localhost/aahrs-new/hospital_Controller/doctors';
                    //} else {
...

    {
        ...
        if (!$this->input->post()) {
            ...
            $data = array(
                ...
                'dept_name' => implode(',',$this->input->post('dept[]'))
...
// print_r($data);die;
      ...
      public function checkDept(){
        ...
        $test = $this->HospitalView_Model->checkdept($this->input->get('q'));

        if ($test) {

            echo "<label class='text-danger'>Department already added</label>";
...

    {
        ...
        $confirmed = $this->hospitalView_Model->delDept($hos_id, $id);
...

    {
        ...
        if ($_FILES['picture']['name']) {
            ...
            if (!$this->upload->do_upload('cover_pic')) {
                ...
                $upload_data = $this->upload->data();
                $image_path1 =  $upload_data['file_name'];
                $data['cover_pic'] = $image_path1;
...
<?php
    ...
    public function fetchDocApptName()
    {
        if (isset($_POST['action'])) { 
            $search=NULL;
                ...
                $search = $_POST['search'];
                $this->load->model('hospitalView_Model');
            echo $this->hospitalView_Model->fetchDocApptName($search);
        }
    }
...

    {
        ...
        $depts = $this->hospitalView_Model->deptsAll($hos_id);
    ...
    public function checkDoc()
    {
        ...
        $test = $this->hospitalView_Model->checkdoc($this->input->get('q'));
...

    {
        ...
        if ($this->input->post()) {
        //   $data = implode(', ',$this->input->post('dosage'));
        //   echo $data;die;
             $data = array(
            ...
            'dosage' => implode(', ',$this->input->post('dosage')),
            ...
            $this->db->insert('hospital_patients_medication', $data);
        }
                    ...
                    <body>
                        ...
                        <table style="border-bottom:1px solid gray">
                            ...
                            <tr>
                                ...
                                <td style="border:none;"><img class="avatar" src="' . base_url('images/' . $fetchdatasendemail[0]['logo']) . '" style="height:100px; width:100px;"></td>
    ...
    public function check_regis_num(){
       ...
       // echo $regis;
        ...
        $result = $this->doctors_Model->get_state_council($council, $regis);
